# Tests de régression des skills — Plan

## Objectif

Disposer d'un CDC fixe (cahier des charges) qui, soumis aux skills `sdd-spec-write` et `sdd-uc-spec-write`, produit un SPEC.md quasi-déterministe. Permet de détecter les régressions lors des modifications de skills.

## Approche

Les skills sont conversationnels (dialogue multi-tours). Pour tester automatiquement, le CDC doit fournir **toutes les réponses** que le skill poserait normalement. Le prompt de test demande au skill de produire le SPEC.md directement, sans poser de questions.

### Structure du test

```
tests/
├── cdc-bibliotheque.md              # CDC fixe (entrée)
├── prompt-sdd-spec-write.md         # Prompt de test pour la variante exigences
├── prompt-sdd-uc-spec-write.md      # Prompt de test pour la variante UC
├── reference/
│   ├── spec-write-SPEC.md           # SPEC.md de référence (variante exigences)
│   └── uc-spec-write-SPEC.md        # SPEC.md de référence (variante UC)
└── output/                          # Sorties des exécutions de test (gitignored)
```

### Processus de test

1. Soumettre le CDC au skill avec le prompt de test
2. Comparer la sortie au SPEC.md de référence
3. Les différences signalent une régression ou une amélioration à valider

### Critères de comparaison

La comparaison ne peut pas être un `diff` strict (la formulation variera). Vérifier plutôt :
- Présence de tous les identifiants attendus (UC-xxx, EXG-xxx, CA-xxx, RG-xxx, ENF-xxx)
- Structure des sections (toutes les sections du template sont présentes)
- Cohérence des règles de gestion (valeurs numériques identiques)
- Complétude de la checklist de livraison

---

## CDC de test — Bibliothèque municipale
...
```
Voici un cahier des charges complet pour le projet BiblioSoft.
Toutes les informations nécessaires sont fournies dans le document :
contexte, contraintes, domaines fonctionnels, exigences non fonctionnelles,
hors périmètre, architecture.

Produis le SPEC.md directement en suivant le processus du skill,
sans poser de questions. Considère chaque réponse du CDC comme validée.
Si une information manque, signale-le dans le document final plutôt
que de t'arrêter.

[contenu de cdc-bibliotheque.md]
```

### Pour sdd-uc-spec-write

```
Voici un cahier des charges complet pour le projet BiblioSoft.
Toutes les informations nécessaires sont fournies dans le document :
contexte, acteurs, contraintes, cas d'utilisation détaillés, exigences
non fonctionnelles, hors périmètre, architecture, diagramme de contexte.

Produis le SPEC.md structuré par cas d'utilisation directement en suivant
le processus du skill, sans poser de questions. Considère chaque réponse
du CDC comme validée. Si une information manque, signale-le dans le
document final plutôt que de t'arrêter.
...
### Manuelle (première version)

```bash
# 1. Lancer Claude Code avec le skill et le CDC
# 2. Comparer visuellement la sortie avec la référence
# 3. Mettre à jour la référence si l'évolution est voulue
```

### Automatisée (future)

Utiliser Claude Code en mode non-interactif avec `--print` ou via l'API :

```bash
claude --print -s "$(cat tests/prompt-sdd-spec-write.md)" > tests/output/spec-write-SPEC.md
```

Puis comparer avec un script qui vérifie :
- Présence des identifiants attendus (liste fixe extraite de la référence)
- Structure des sections (grep des titres de section)
- Valeurs des règles métier (grep des chiffres clés : 5 emprunts, 21 jours, 0.20€, etc.)
